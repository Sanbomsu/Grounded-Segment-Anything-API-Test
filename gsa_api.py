...
def get_mask_pixels(mask):
  # mask to image
  mask = np.array(mask.tolist()[0], dtype=np.uint8)
  mask[mask == True] = 255
  mask[mask == False] = 0
  cnts, _ = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
  cv2.drawContours(mask, cnts, -1, (255, 0, 0), 3)

  all_pixels = []

  for i in range(0, len(cnts)):
    for j in range(0, len(cnts[i])):
      all_pixels.append(list(cnts[i][j])[0].tolist())
...
def load_model_hf(model_config_path, repo_id, filename, device='cpu'):
  ...
  cache_file = 'groundingdino_swint_ogc.pth'
...
def get_grounding_output(model, image, caption, box_threshold, text_threshold,
  ...
  for logit, box in zip(logits_filt, boxes_filt):
    pred_phrase = get_phrases_from_posmap(logit > text_threshold, tokenized,
                                          tokenlizer)
    if with_logits:
...
def run_grounded_sam(image_path, text_prompt, task_type, inpaint_prompt,
                     box_threshold, text_threshold, image_name):
  ...
  boxes_filt, pred_phrases = get_grounding_output(model, image, text_prompt,
  ...
  if task_type == 'seg' or task_type == 'inpainting':
    ...
    transformed_boxes = predictor.transform.apply_boxes_torch(boxes_filt,
                                                              image.shape[:2])
  ...
  if task_type == 'det':
    ...
    return {'data': get_img_b64(image_with_box), 'extra': extra}
    ...
    for mask in masks:
      ...
      contours.append(get_mask_pixels(mask.cpu().numpy()))
    ...
    # image_result = cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2RGB)
    # return image_result
    extra['contours'] = contours

    # 将检测后的图片转换成png格式后输出base64
    return {'data': get_img_b64(Image.open(image_path)), "extra": extra}
    ...
    image = \
      pipe(prompt=inpaint_prompt, image=image_pil, mask_image=mask_pil).images[
        0]
    ...
    # 将检测后的图片转换成png格式后输出base64
    return {'data': get_img_b64(image), "extra": extra}

  else:
    print("task_type:{} error!".format(task_type))


def run_grounded_sam_bbox(image_path, text_prompt, task_type, inpaint_prompt,
                          box_threshold, text_threshold, image_name):
  assert text_prompt, 'text_prompt is not found!'
...
async def main(data: Item):
  ...
  res = run_grounded_sam(image_path=img_path, text_prompt=data.text_prompt,
                         task_type=data.task_type, inpaint_prompt='',
...
async def crop(img):
  ...
  res = run_grounded_sam_bbox(image_path=img_path, text_prompt='a bottle',
                              task_type='det', inpaint_prompt='',
...
def onnx_show_points(coords, labels, ax, marker_size=375):
...
def run_grounded_sam_onnx(image_path, text_prompt, task_type, inpaint_prompt,
                          box_threshold, text_threshold, image_name):
  assert text_prompt, 'text_prompt is not found!'

  # 1.grounded dino
  # load image
  image_path_ = Image.open(image_path)
  image_pil, image = load_image(image_path_.convert("RGB"))
  # load model
  model = load_model_hf(config_file, ckpt_repo_id, ckpt_filenmae)
  ...
  # 2.segment-anything
  image = cv2.imread(image_path)  # image0
  image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

  onnx_model_quantized_path = "sam_vit_h_4b8939_quantized.onnx"
  onnx_model_path = onnx_model_quantized_path

  # 2. Using an ONNX model
  ...
  # grounded dino results
  input_point = np.array(boxes_filt)
  input_label = np.array([1] * len(boxes_filt))
  ...
  onnx_label = np.concatenate([input_label, np.array([-1])], axis=0)[None,
               :].astype(np.float32)
  onnx_coord = predictor.transform.apply_coords(onnx_coord,
                                                image.shape[:2]).astype(
      np.float32)
  ...
  plt.imshow(image)
  for mask in masks[0]:
    onnx_show_mask(mask, plt.gca())
  for box in boxes_filt:
    onnx_show_box(box, plt.gca())
  ...
  return {'data': get_img_b64(Image.open(image_path))}


class GsaOnnxParams(BaseModel):
  pic_url: str
  task_type: str
  text_prompt: str
  box_threshold: float
...
@app.post("/gsa_onnx")
async def gsa_onnx(data: GsaOnnxParams):
  parser = argparse.ArgumentParser("Grounded SAM demo", add_help=True)
  parser.add_argument("--debug", action="store_true", help="using debug mode")
  parser.add_argument("--share", action="store_true", help="share the app")
  args = parser.parse_args()
  ...
  # 图片处理
  # image_path, text_prompt, task_type, inpaint_prompt, box_threshold, text_threshold
  res = run_grounded_sam_onnx(image_path=img_name, text_prompt=data.text_prompt,
                              task_type=data.task_type, inpaint_prompt='',
                              box_threshold=data.box_threshold,
                              text_threshold=data.text_threshold,
                              image_name=img_name)
...
if __name__ == "__main__":
  uvicorn.run(app='gsa_api:app', host='0.0.0.0', port=7590, reload=True)
